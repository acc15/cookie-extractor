...
fun Path.mkparents() = this.apply { parent?.also { Files.createDirectories(it) } }

fun writeTextAndLog(path: Path, what: String, text: String) {
    if (Files.exists(path)) {
        logger.lifecycle("$what at \"$path\" already exists, keeping existing...")
    } else {
        Files.writeString(path.mkparents(), text)
                ...
                clients:
                    main:
                        files:
                            sh:
                                format: shell
                                path: cookies/main.sh
                            json: 
                                format: json_map
                                path: cookies/main.json
            """.trimIndent()
        )
    }
